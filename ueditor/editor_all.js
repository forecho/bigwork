    ...
    Editor.prototype = /**@lends baidu.editor.Editor.prototype*/{
        ...
        _setup: function ( doc ) {
            ...
            doc.write( ( ie && browser.version < 9 ? '' : '<!DOCTYPE html>') +
                ...
                ( options.iframeCssUrl ? '<link rel="stylesheet" type="text/css" href="' + utils.unhtml( options.UEDITOR_HOME_URL + options.iframeCssUrl ) + '"/>' : '' ) +
                '<style type="text/css">'
            ...
            if(img && /img/i.test( img.tagName ) && img.className != "edui-faked-video" &&!img.getAttribute("word_img") ){
                ...
                if(opt.length == 1){
                    str = '<img src="'+ci.src+'" '+
                            ...
                            ' style="' + (ci['floatStyle']&&ci['floatStyle']!='center' ? 'float:'+ci['floatStyle']+';':'') +'border:'+(ci.border||'0') + 'px solid #000" ' +
                            (ci.title?' title="'+ci.title+'"':'') + ' border="'+ (ci.border||0) + '" />';
                    ...
                    getClipboardData.call( me, function( div ) {
                        ...
                        if ( modify_num.flag && me.ui){
                         ...
                         if ( word_img_flag.flag && !me.queryCommandState("pasteplain") && me.ui){
                            me.ui.showToolbarMsg( me.options.messages.pasteWordImgMsg );
                             word_img_flag.flag = '';
                            //trance:为了解决fireevent  (selectionchange)事件的延时
                            setTimeout(function(){
                                me.addListener("selectionchange",hideMsg);
    ...
    me.addListener("ready",function(){
        if(!document.getElementById("syntaxhighlighter_js")){
            var obj = {
                ...
                src : me.options.UEDITOR_HOME_URL+"third-party/SyntaxHighlighter/shCore.js",
                tag : "script",
                type : "text/javascript",
                defer : "defer"
            };
        ...
        if(!me.document.getElementById("syntaxhighlighter_css")){
            var obj = {
                ...
                rel : "stylesheet",
                type : "text/css",
                href : me.options.UEDITOR_HOME_URL+"third-party/SyntaxHighlighter/shCoreDefault.css"
            }
            ...
            str = str.replace( /(<[a-z][^>]*)\sstyle=(["'])([^\2]*?)\2/gi, function( str, tag, tmp,style ) {
                ...
                for ( var i = 0; i < s.length; i++ ) {
                    ...
                    if ( parts.length == 2 ) {
                        ...
                        //pt 转换成为px
                        if(/pt$/.test(parts[1])){
                            parts[1] = Math.round(parseFloat(parts[1]) / 0.75) + 'px';
                        }
        ...
        switch(node.tag){
            case 'img':
                if(node.attributes.src && /^(?:file)/.test(node.attributes.src)){
                    ...
                    node.attributes.src =  editor.options.UEDITOR_HOME_URL + 'themes/default/images/localimage.jpg';
                ...
                if (j) while (j --) {
                    ...
                    while (k --) {
                        ...
                        if(tableobj.cellpadding){
                            style = 'style="padding:'+tableobj.cellpadding + 'px"'
                        }
                        cols[k] = '<td '+ style +' width=' + Math.floor((tableobj.width || 500) / tableobj.numCols) + ' height="'+(this.options.tdHeight||50)+'">' +
                            //trace: IE6下占位符
        ...
        me.commands['edittable'] = {
            ...
            execCommand: function (cmdName, tableobj) {
                ...
                if (table) {
                    ...
                    table.style.borderWidth =tableobj.border+"px";
